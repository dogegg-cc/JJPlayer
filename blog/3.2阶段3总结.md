...
2. **硬核音频解码与 libswresample 物理重采样**：在 `JJFFmpegCore` 中激活音频解码上下文，使用 `libswresample` 将任意输入的音频采样格式（Planar/FLTP 等）、采样率动态重采样为 iOS 系统原生声卡最契合 the 物理参数：**`44100Hz`, `16-bit`, `双声道 (立体声)`, `Packed (S16) 交错型` PCM 数据**。
...
5. **os_unfair_lock 物理防反转重构**：弃用存在优先级反转（Priority Inversion）隐患的传统 `NSLock`，在 Swift 层面封装 Darwin 原生轻量级 **`os_unfair_lock`**。保证实时声卡回调（High QoS）与解码线程（Low QoS）之间的高频同步极速互斥，且内核自适应调度防卡顿。

---

## 💥 核心避坑与架构设计研讨

在手搓 iOS 底层音频播放器时，有三大隐藏在多媒体开发深处的隐形杀手：
...
### 3. 实时线程与解码线程同步（锁与优先级反转巨坑）
* **现象**：`AudioQueue` 的系统回调线程运行在系统极高级别的实时 QoS 下，而解码线程运行在普通的交互 QoS 下。如果使用传统的 `NSLock`，当解码线程正在持有锁追加数据时，被其他中等优先级线程抢占 CPU 挂起，声卡回调线程再去索要数据就会被迫阻塞。高优先级等待低优先级释放，就会发生优先级反转，造成音频断流与杂音。
* **物理防线**：**Darwin os_unfair_lock 物理重构**。
  弃用 `NSLock`，采用 iOS 原生的低级互斥锁 `os_unfair_lock`。它不仅比 `NSLock` 更轻量（4字节，无 Objective-C runtime 派发开销），更关键是**由 Darwin 内核自动在等待发生时提升低优先级线程的执行优先级（自适应优先级继承）**，彻底终结了实时场景下的优先级反转与死锁隐患。
...
#### [NEW] [JJUnfairLock.swift](file:///Users/jiao/Desktop/Project/JJPlayer/JJPlayerKit/Sources/JJPlayerKit/Utils/JJUnfairLock.swift)
* 封装基于 Darwin 底层原生 `os_unfair_lock` 的互斥同步锁。
* 设计细节：
  * 大小仅 4 字节，对比高级 `NSLock` 拥有极致性能并杜绝闭包开销。
  * 原生支持优先级继承防御优先级反转，特别适用于多媒体播放的实时与异步交互同步。
...
* 引入基于 `JJUnfairLock` 锁保护的音频缓冲追加队列：
  ```swift
  private var audioBuffer = Data()
  private let audioLock = JJUnfairLock() // 💥【重大优化：使用 Darwin 原生高性能 os_unfair_lock 杜绝优先级反转与卡顿】
...
5. **验证重点**：
   ...
   * 监控实时音频回调与解码互斥对锁的争抢情况，确认有锁等待发生时无线程饿死或 CPU 异常飙高情况。
